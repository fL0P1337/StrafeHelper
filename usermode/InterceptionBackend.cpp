...
bool InterceptionBackend::Initialize() noexcept {
  ...
  if (!ResolveApi()) {
    Utils::LogError("[InterceptionBackend] failed to load Interception runtime "
                    "or resolve required exports.");
    Shutdown();
    return false;
  }
  ...
  if (!context_) {
    Utils::LogError(
        "[InterceptionBackend] interception_create_context failed.");
    Shutdown();
    return false;
  }
  ...
  if (!EnumerateKeyboards()) {
    Utils::LogError(
        "[InterceptionBackend] failed to enumerate keyboard devices.");
    Shutdown();
    return false;
  }
  ...
  Utils::LogInfo(
      "[InterceptionBackend] Initialized. Keyboards enumerated: %llu",
      static_cast<unsigned long long>(keyboardDevices_.size()));
  return true;
}
...
bool InterceptionBackend::ResolveApi() noexcept {
  auto tryLoad = [this](const wchar_t *dllName) -> bool {
    ...
    Utils::LogInfo("[InterceptionBackend] Loaded %ls", dllName);
    return true;
  };
  ...
  auto resolve = [this](auto &target, const char *name) -> bool {
    ...
    if (!target) {
      Utils::LogError(
          "[InterceptionBackend] missing export '%s' (GetLastError=%lu)", name,
